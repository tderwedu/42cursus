...
/*   Updated: 2021/05/19 09:42:36 by tderwedu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_mini_map(t_mlx *mlx)
{
	...
	mmap_draw_mmap(mlx, &map);
	mmap_draw_edge(mlx, &map);
	mmap_draw_player_pos(mlx, img, &map);
}
...
static inline void	mmap_get_rgb(t_mlx *mlx, int val, int y_draw, int x_draw)
{
	if (val == m_empty)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_FLOOR);
	else if (val == m_wall || val == m_secret)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_WALL);
	else if (val == m_spr)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_SPRITE);
	else if (val == m_knife)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_KNIFE);
	else if (val == m_door)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_DOOR);
	else if (val == m_kfc)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_KFC);
	else if (val < 0)
		mmap_draw_square(mlx, y_draw, x_draw, RGB_EMPTY);
}
...
void	mmap_draw_mmap(t_mlx *mlx, t_mini *map)
{
	...
	while (++y < map->y_max)
	{
		...
		mmap_draw_square(mlx, y_draw, 1, RGB_EDGE);
		x = map->x_min - 1;
		while (++x < map->x_max)
		{
			x_draw = x + 2 - map->x_min;
			mmap_get_rgb(mlx, mlx->map[y][x], y_draw, x_draw);
		}
		x_draw = x + 2 - map->x_min;
		mmap_draw_square(mlx, y_draw, x_draw, RGB_EDGE);
	}
}
