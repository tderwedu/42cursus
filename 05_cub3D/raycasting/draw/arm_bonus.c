...
/*   Updated: 2021/05/11 12:01:52 by tderwedu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	arm(t_mlx *mlx)
{
	...
	box = (t_loop){0, mlx->height, mlx->height, 0, mlx->tex[i].height, 0, 0, 0, 0,
		mlx->tex[i].width, 0x22B14C};
	...
	if (mlx->strike > 0) //TODO: improve and ADD object detection
		mlx->strike++;
	if (mlx->strike > 9)
		mlx->strike = 0;
}
